...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # Текущее AST (последний успешный анализ)
        self.current_ast = None
        self.current_ast_json = None
    ...
    def _setup_central_area(self) -> None:
        ...
        # Текстовый виджет для отображения AST
        from PySide6.QtWidgets import QPlainTextEdit
        from PySide6.QtGui import QFont
        self.output_ast = QPlainTextEdit()
        self.output_ast.setReadOnly(True)
    ...
    def _setup_actions(self) -> None:
        ...
        self.action_show_ast.triggered.connect(self.show_ast_graphics)
    ...
    def start_analyzer(self) -> None:
        ...
        # Семантический анализ (строит AST)
        ...
        # Сохраняем AST для возможной визуализации
        self.current_ast = ast
        self.current_ast_json = ast_json
        
        # Очищаем вкладку AST и заполняем её
        self.output_ast.clear()
        if ast_text:
            self.output_ast.setPlainText(ast_text)
        
        # Выводим общее количество ошибок в лог
        if total_errors == 0:
            log_text = "Синтаксический и семантический анализ завершены успешно. Ошибок не обнаружено."
            self.output_log.setPlainText(log_text)
            # Переключаем на вкладку AST, если нет ошибок
            self.output_tabs.setCurrentWidget(self.output_ast)
        else:
    ...
    def show_ast_graphics(self) -> None:
        ...
        if self.current_ast is None:
            QMessageBox.information(self, "Нет данных", "Сначала выполните анализ кода (Пуск).")
            return
        show_ast_graphics(self.current_ast, self)
